...
def _load_csv(path: str):
    ...
    with open(path, "r", newline="") as f:
        ...
        for row in reader: xs.append(float(row[0])); ys.append(float(row[1]))
    return xs, ys
...
def main() -> None:
    ...
    plt.xlabel("λ (STL weight)"); plt.ylabel("Robustness  G(mean_x u ≤ u_max)")
    plt.title("Diffusion 1D: STL Weight Sweep"); plt.grid(True, linestyle="--", alpha=0.3)
