...
def _train_and_measure_once(*, stl_weight: float, epochs: int, seed: int,
                            n_x: int, n_t: int, u_max: float, alpha: float,
                            stl_temp: float, stl_spatial: str,
                            results_dir: str, tag_suffix: str = "") -> float:
    try:
        import torch  # type: ignore
        # Import inside try so a missing optional dep leads to proxy path.
        ...
        cfg: dict[str, Any] = {
            ...
            "grid": {"n_x": int(n_x), "n_t": int(n_t), "x_min": 0.0, "x_max": 1.0, "t_min": 0.0, "t_max": 1.0},
        ...
        if stl_spatial == "mean":
            ...
            try:
                from physical_ai_stl.monitoring.stl_soft import softmax as stl_softmax  # type: ignore
            ...
            raise ValueError(f"Unknown stl_spatial={stl_spatial!r} (expected 'mean'|'softmax'|'amax').")
...
def _parse_args(argv: Iterable[str] | None) -> Args:
    ...
    ap.add_argument("--weights", type=float, nargs="+", default=[0.1, 1.0, 10.0],
                    help="List of STL penalty weights (λ values) to sweep.")
    ap.add_argument("--epochs", type=int, default=100,
                    help="Epochs per trial (kept modest for speed).")
    ...
    ap.add_argument("--stl-temp", type=float, default=0.1, dest="stl_temp",
                    help="Temperature for soft mins/maxes in STL semantics.")
    ap.add_argument("--stl-spatial", type=str, default="mean", choices=["mean", "softmax", "amax"],
    ...
    ap.add_argument("--results-dir", type=str, default="results",
                    help="Where experiment artifacts are written.")
    ns = ap.parse_args(argv)
    return Args(
        weights=list(ns.weights), epochs=int(ns.epochs), out=str(ns.out), seed=int(ns.seed),
        n_x=int(ns.n_x), n_t=int(ns.n_t), u_max=float(ns.u_max), alpha=float(ns.alpha),
        stl_temp=float(ns.stl_temp), stl_spatial=str(ns.stl_spatial),
        repeats=int(ns.repeats), results_dir=str(ns.results_dir),
    )


def train_once(stl_weight: float = 1.0, epochs: int = 100, seed: int = 0,
               *, n_x: int = 64, n_t: int = 64, u_max: float = 1.0, alpha: float = 0.1,
               stl_temp: float = 0.1, stl_spatial: str = "mean",
               results_dir: str = "results") -> float:
    return _train_and_measure_once(
        stl_weight=stl_weight, epochs=epochs, seed=seed,
        n_x=n_x, n_t=n_t, u_max=u_max, alpha=alpha,
        stl_temp=stl_temp, stl_spatial=stl_spatial, results_dir=results_dir,
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    for w in args.weights:
        ...
        for i in range(args.repeats):
            r = train_once(
                stl_weight=float(w), epochs=args.epochs, seed=args.seed + i,
                n_x=args.n_x, n_t=args.n_t, u_max=args.u_max, alpha=args.alpha,
                stl_temp=args.stl_temp, stl_spatial=args.stl_spatial,
